{!! Form::model($model, ['method' => 'PUT', 'route' => [$update_route, $model->{$model->getRouteKeyName()}], 'class' => 'Formulario-Horizontal']) !!}
  ...
  <section>
    ...
    <div class="Fila {{ $errors->has('country_id') ? 'El--con-error' : '' }}">
      ...
      <div class="Grupo-Formulario Col-Tablet-8">
        {!! Form::select('country_id', isset($countries) ? $countries : [],  null, ['placeholder' => 'Seleccioná un páis', 'id' => 's2Country']) !!}
    ...
    <div class="Fila {{ $errors->has('zip') ? 'El--con-error' : '' }}">
      {!! Form::label('zip', 'Codigo postal', ['class' => 'Grupo-Formulario Col-Tablet-2']) !!}
      <div class="Grupo-Formulario Col-Tablet-8">
        {!! Form::text('zip', null) !!}
        @if($errors->has('zip'))
        <span class="Mensaje-Validacion">{{ $errors->first('zip') }}</span>
        @endif
      </div>
    </div>
    <hr>
  ...
  <section class="Botonera-Inferior-Derecha">
      <button type="submit" class="Btn-Primario"><i class="Icono Icono-Izquierda"></i>Guardar</button>
